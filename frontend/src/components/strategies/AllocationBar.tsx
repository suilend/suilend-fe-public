...
function AllocationBar({ vault }: AllocationBarProps) {
  ...
  const allocationSegments = (vault.obligations.map((obligation, index) => ({
  ...
  return (
    <div className="h-[16px] w-full">
                      <div className="flex h-full w-full flex-row">
                        {allocationSegments.filter((seg) => !seg.percent.isNaN()).map((seg) => (
                  <Tooltip
                    content={
                      <div className="flex flex-col gap-1">
                          <div key={seg.name} className="flex flex-row items-center justify-between gap-4">
                          ...
                          <div key={seg.name} className="flex flex-row items-center justify-between gap-4">
                            ...
                            <TBody>{formatToken(seg.tvlAmount)} {vault.baseCoinMetadata?.symbol}</TBody>
                          </div>
                          {seg.apr && <div key={seg.name} className="flex flex-row items-center justify-between gap-4">
                  ...
                  >
                          <div
                            key={seg.name}
                            className={cn(
                              "h-full bg-muted/20 hover:scale(1.1) transition-all duration-300 cursor-pointer flex items-center justify-center overflow-hidden hover:scale-105",
                            )}
                            style={{ width: `${seg.percent}%`, backgroundColor: seg.color }}
                          ><TBody className="text-center text-[9px] text-ellipsis overflow-hidden whitespace-nowrap px-2">{seg.name} {seg.percent ? ` (${formatPercent(seg.percent, { dp: 0 })})` : ''}</TBody></div>
                  </Tooltip>
                        ))}
                      </div>
                    </div>
